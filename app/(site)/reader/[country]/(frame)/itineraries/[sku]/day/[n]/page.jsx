 ...
 * on a phone. The sample day is open to everyone; the rest show the buy
 * box in place of the rail.
 */
export default async function ReaderDay({ params }) {
  ...
  const open = owned || (sampleDay != null && page.dayFrom <= sampleDay && sampleDay <= page.dayTo);
  trackReaderView(country, "day", { route: skuSlug, day: page.dayFrom, access: owned });
  ...
  const pins = pinsForMap(country, dayPlaces, { numbered, link: owned });
    ...
    <>
      ...
      <div className="mt-4 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
        <div className="min-w-0">
          <header>
            <p className="font-sans text-[11px] font-bold uppercase tracking-[0.14em] text-brand-terracotta">
              ...
              {!owned && open ? " · Sample day" : ""}
            </p>
            <h2 className="mt-1 text-3xl leading-tight">{titleCase(page.title)}</h2>
            {page.subtitle ? <p className="mt-2 text-[17px] leading-snug text-slate-700">{page.subtitle}</p> : null}
          </header>

          {!open ? (
            <div className="mt-6">
              <LockedBox data={data} what={`${dayBadge(page)} · ${titleCase(page.title)}: the hour-by-hour plan, its stops and its warnings.`} />
            </div>
          ) : stub ? (
            ...
            <section className="mt-6">
              <DayView
                rail={<Timeline country={owned ? country : null} page={page} places={places} />}
                list={<StopList country={country} page={page} places={places} optionalPins={optional} />}
              />
            </section>
          )}

          {open && page.callouts.length ? (
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          {open && pins.length ? (
            <>
              <div className="h-[320px] lg:h-[calc(100vh-8rem)]">
