...
## Structured guide content (Stage 1, Fiji pilot)

Paid structured content lives in a private Neon Postgres (`db/schema.js`,
`DATABASE_URL`), because the Sanity dataset is publicly readable. Authoring
truth is repo YAML: `content/countries/<c>/places.yaml` (machine-owned —
regenerated by `npm run import:master` from the founder's master workbook;
never hand-edit it, and never treat it as an append-only deliverable, git is
its history) and each guide's `sku.yaml` (repo-owned authoring truth, edit
freely). `npm run publish:sku -- --slug <slug>` renders YAML into Postgres;
`npm run check:sku` is the drift checker, same philosophy as `check:guides`.
The gated reader lives at `/guides/<slug>/read` behind the purchase token
(`app/_lib/guideAccess.js`); `place.internal_notes` and `place.booking_url`
must never be serialised to any client — go-slugs are what render.
