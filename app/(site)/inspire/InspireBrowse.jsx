...
// The trip row under the boxes heading (founder mockup 2026-08-17). A trip is
// one real journey, not a category: the stories that came out of a single
// continuous run.
//
...
export default function InspireBrowse({
  ...
  // (founder 2026-08-17). A passive scroll listener rather than an
  // IntersectionObserver: the header measures its own scroll the same way,
  // and this only fires the event when the answer actually flips, so the
  // header re-renders twice per visit rather than on every frame.
  useEffect(() => {
  ...
  // Hovering the header search opens the full filter box (founder
  // 2026-08-17). SiteHeader reports that hover through a window event rather
  // than importing anything from this page — the same loose coupling the home
  // hero uses to tell the header whether to show itself.
  const openPanel = () => {
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="w-full min-w-0 space-y-6">
        ...
        {/* Search + facets in one frame (founder mockup 2026-08-17). Style is
            the story's activity category — our own vocabulary, not a
            hand-written list, so a facet appears only once a story carries it
            and the row tracks the library. */}
        <div
          ref={cardRef}
