#!/usr/bin/env node
/**
 * One-shot: create the Tuvalu 2-day guide story doc in Sanity.
 *
 * Cloned from create-iceland-south-coast-guide.mjs (the newest sibling, per the
 * production playbook). Every fact below comes from the sellable deck (the
 * founder's final pptx, QA-gated 2026-08-13), the country master v6, or Content
 * Plan v54 - nothing is written from memory.
 *
 * Title, subtitle, meta description and prices verbatim from the Content Plan
 * Guides sheet (founder price 2026-08-13: EUR 29 / GBP 29 / CHF 29 / USD 35,
 * customPrices, no tier matches).
 *
 * estimatedCost is NOT set: the deck quotes one all-in figure (~EUR 1,065 pp
 * incl. the Fiji flights), not a LEAN..SPLURGE band, and the site field wants a
 * range. The cost story lives in the deck and in body copy.
 *
 * GEO/SEO intent (destination-page playbook s4/s6, applied to the sales page):
 * Tuvalu is searched as a *question* before it is searched as a place -
 * "least visited country in the world" is the head query, then "how to get to
 * Tuvalu" / "is Tuvalu worth visiting" / "things to do". appearsInSearches +
 * keywords target those phrasings; searchSynonyms carries Funafuti / Fongafale
 * / Ellice Islands so the doc matches however the atoll is named.
 *
 * Idempotency: createOrReplace on a fixed _id. Run --dry-run first.
 *
 * Usage:
 *   node --env-file=.env.local scripts/create-tuvalu-2-days-guide.mjs [--dry-run]
 */
...
const ASSETS = String.raw`C:\Users\pauli\AppData\Local\Temp\claude\C--Users-pauli-Desktop-TestedRoutes---Website\cc2ced33-6ac5-482f-93d1-0015266a1930\scratchpad\guide-assets-tuvalu-2d`;
const PDF = String.raw`C:\Users\pauli\Desktop\TestedRoutes - Website\content\countries\tuvalu\guides\tuvalu-2-days\TestedRoutes_Tuvalu_Guide_2_Days.pdf`;
...
// Alt text written from the actual frames (contact strip), not from memory.
const hero = await img(
  "hero.jpg",
  "Funafuti's runway tarmac at golden hour with the terminal building behind",
);
const galleryDefs = [
  ["g1.jpg", "The Fiji Airways ATR 72 parked at Funafuti's terminal - the only way in"],
  ["g2.jpg", "Two kids cycling across the bare coral flat under a huge sky"],
  ["g3.jpg", "The rubble spit running out from Fongafale's southern arm"],
  ["g4.jpg", "The road north through a green tunnel of palms and pandanus"],
  ["g5.jpg", "Lunch at the Funafuti Lagoon Hotel, the country's one working restaurant"],
  ["g6.jpg", "The rusting tugboat beached on the lagoon shore"],
];
const gallery = [];
